...
class CustomFields
{
	...
	private static function buildTreeCats($data, array $checkedArr = [], array $relCategories = [], int $parent_id = 0): string {
		$treeItem = '';
				...
				if(empty($treeItemChilds)){
					$treeItem .= '<div class="oa-tree-leaf">
						<div class="oa-tree-label ' . ($rel_value ? 'relation-active' : '') . '">
							...
							<label>
								<input type="checkbox" class="oa-tree-cb-cat ui-ctl-element" name="categories[]" value="' . $item['id'] . '"' . $checked . '/>
